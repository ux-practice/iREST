...
export function getBulkResponse(req, res) {
  ...
  try {
    successResponse = successResponse.replaceAll(`:"$data"`,`:${JSON.stringify(req.requestData.inputData)}`)
    successResponse = JSON.parse(successResponse)
  } catch (error) {
    logger.error(error)
    return res.status(500).send({message:serverErrorMessage,status: 500,error})
  }
  return res.status(parseInt(statusCode,10)).send(Object.keys(successResponse).length === 0 ? req.requestData.inputData : successResponse)
