...
export const customFormat = ({label, timestamp, level, message, ...rest}) => `${label} ${timestamp} ${level}: ${message} ${JSON.stringify(rest) === "{}" ? "" : JSON.stringify(rest)}`
  ...
  const logger = createLogger(process.env.NODE_ENV === 'production' ? {transports: [ new transports.Console({level: 'error',format: combine(colorize(), combine(
    colorize({
      all: true,
    }),
    label({
    ...
    printf(customFormat)
  )),}),new transports.Console({level: 'http',format: combine(colorize(), combine(
    colorize({
      all: true,
    }),
    label({
